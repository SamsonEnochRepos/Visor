"""
runtime.py — Application runtime for VISOR spatial computing platform.
...
This module holds all the orchestration logic. The project-root
``main.py`` is a thin launcher that puts ``src/`` on ``sys.path`` and
calls :func:`main` here.

Architecture v2.0: Uses the visor/ package with async pipeline,
landmark-based gesture classifier, intent engine, and spatial UI.

Path note: all on-disk assets are resolved relative to ``_ROOT`` (the
project root, three directories up from this file):
    _ROOT/assets/hand_landmarker.task
    _ROOT/models/<VOSK_MODEL_PATH>
    _ROOT/config.json
    _ROOT/touchless_os.log
"""
...
# ---------------------------------------------------------------------------
#  Project root — this file is at <root>/src/visor/runtime.py
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
_LOG_FILE = os.path.join(_ROOT, "touchless_os.log")
...
def _print_startup_checklist(cfg: Config) -> None:
    ...
    # Hand model
    model_path = os.path.join(_ROOT, "assets", "hand_landmarker.task")
    ...
    vosk_path = os.path.join(
        _ROOT, cfg.get_value("VOSK_MODEL_PATH", "models/vosk-model-small-en-us-0.15")
    )
